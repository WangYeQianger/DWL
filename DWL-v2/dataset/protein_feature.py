...
def get_protein_feature_mda(pocket_mol, top_k=30):
    with torch.no_grad():
        pure_res_lis, pure_res_key, pure_resname, seq, node_s, X_ca, X_n, X_c, xyz_full = [], [], [], [], [], [], [], [], []
        for res in pocket_mol.residues:
            try:
                ...
                others = res_atoms.select_atoms("not name CA C N O")
                dis_max = dists.max()
                dis_min = dists.min()
                ...
                pure_res_key.append(f'{res.atoms.chainIDs[0]}-{res.resid}{res.icode}') 
                pure_resname.append(f'{res.resname}{res.resid}{res.icode}')
                res_pos = np.concatenate([n.positions, ca.positions, c.positions, o.positions, others.positions], axis=0)
                xyz_full.append(torch.from_numpy(np.pad(res_pos[:RES_MAX_NATOMS], ((0, RES_MAX_NATOMS-min(res_pos.shape[0], RES_MAX_NATOMS)), (0, 0)), 'constant', constant_values=np.nan)))
            except:
                continue
        ...
        xyz_full = torch.stack(xyz_full, dim=0)
