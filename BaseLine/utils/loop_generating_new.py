...
if torch.cuda.is_available():
    ...
    my_device = 'cpu'
# model - Remove DataParallel wrapper since we're using single GPU
...
for re in range(1):
    ...
    with torch.no_grad():
        for idx, data in enumerate(tqdm(test_dataloader)):
            ...
            rmsd_loss, mdn_score, pos_pred = model.forward(data, scoring=args.scoring, dist_threhold=5, train=False)
            # print(rmsd_loss)
            ...
            pos_loss = model.cal_rmsd(pos_true, pos_pred, batch) 
